import os
from pyteomics import mzml, auxiliary, mass, pylab_aux
...
for run_name in run_list:
    ...
    for index, rt_peptide in mass_df.iterrows():
        print(rt_peptide["mass"])
        rt_peptide["run"]=run_name
        for spectrum in f:
            if spectrum["ms level"]==1:
                temp_d = AddPeak(spectrum,
                                 rt_peptide["mz"],
                                 rt_peptide["mzmin"],
                                 rt_peptide["mzmax"],
                                 rt_peptide["mass"],
...
#     temp_peak_rt = rt_intensity_mz[rt_intensity_mz["mass"]==mass_value]
#     if mass_value>1500:
#         temp_peak_rt = rt_intensity_mz[rt_intensity_mz["rt"]>20.0]
#     temp_peak_rt = temp_peak_rt.loc[temp_peak_rt["intensity"].idxmax()]
#     peak_rt = pd.concat([peak_rt, temp_peak_rt.to_frame().T], ignore_index=True)
