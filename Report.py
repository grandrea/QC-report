...
if msScan_produced==True:
    ...
    secondary = ap1.twinx()
    sns.lineplot(data=msScan, x="Retention time", y="MS/MS identification rate [%]", ax=secondary, color="orange", rasterized=True)
    ap1.set_title("TIC & MSMS Id Rate")
    
    sns.lineplot(data=msScan, x="Retention time", y="MS/MS count", ax=ap2, rasterized=True)
    ax2=ap2.twinx()
    sns.lineplot(data=msScan, x="Retention time", y="MS/MS / s", color="orange", ax=ax2, rasterized=True)
    ...
    ap8.table(cellText=summary_table.values, colLabels=summary_table.columns, loc='center', rasterized=True)
    ap8.axis("off")
